    ...
    <div class="animated fadeIn">
        <el-card class="box-card" v-loading="loading" element-loading-text="拼命加载中">
            ...
            <el-form :model="banner" :rules="rules" ref="banner" label-width="100px" class="demo-banner">
                ...
                <el-form-item>
                    <el-button type="primary" @click="updateBanner('banner')">保存</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getBanner: function () {
                axios.get('/api/bar/banner/edit/' + this.$route.params.id ).then(response => {
                    var data = response.data;
                    if (data.code === 200) {
                        this.banner = data.data;
                    } else {
                        this.$message.error(data.message);
                        this.$router.push('/bar/banner');
                    }
            ...
            updateBanner:function (banner) {
                const self = this;
                //验证
                self.$refs[banner].validate((valid) => {
                    if (valid) {
                        self.$confirm('更新 banner 信息, 是否继续?', '提示', {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then(() => {
                            var data = self.banner;
                            //如果有更新图片再转
                            ...
                            axios.post('/api/bar/banner/update/' + self.banner.id, data).then(response => {
                                var data = response.data;
                                if(data.code === 200){
